...
| 里程碑 | 状态 | 说明 |
|--------|------|------|
| 逆向分析（4 篇文档） | ✅ 完成 | 光标覆盖层 / 对话后端 / 多 Agent 运行时 / 悬浮图标 HUD |
| 演示 App（4-Tab） | ✅ 完成 | `App/` 下可编译运行的 SwiftUI 演示，用于验证各模块交互逻辑 |
| **完整功能还原** | ⬜ 进行中 | 将演示代码升级为生产级独立 App：真实光标覆盖层（NSPanel）、真实 SSE 对话（可配 API）、真实 Agent 子进程、真实悬浮 HUD |

`App/` 目录当前是 4 个 Tab 的演示应用（Tab ① 光标覆盖层 / ② 对话后台 / ③ 多 Agent 运行时 / ④ 悬浮图标 HUD），
各 Tab 验证了对应模块的交互与协议逻辑，**下一步将每个模块替换为完整可运行的实现**。
...
| [App/](App/) | SwiftUI 演示 App（4-Tab，可编译运行，演示阶段） | ✅ 演示 |
| [App/](App/) → 完整还原 | 生产级独立 App（真实 NSPanel 光标 / SSE 对话 / Agent 子进程 / 悬浮 HUD） | ⬜ 开发中 |
